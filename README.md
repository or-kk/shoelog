...
- 홈 화면 위젯에서 활성 러닝화 누적 마일리지 상위 3개와 목표 진행률 표시
- 미배정 운동 필터와 비공개 알림
...
휴대폰 홈 화면의 위젯 목록에서 `ShoeLog`를 선택하면 4×2 크기의 마일리지 랭킹을 추가할 수 있습니다. 은퇴하지 않은 러닝화 중 누적 거리가 긴 상위 3개를 표시하며, 각 진행 막대는 해당 러닝화의 목표 거리 대비 진행률입니다.

앱에서 러닝화를 등록·수정하거나 달리기 배정을 바꾸면 실행 중에는 즉시 갱신되고, 시스템은 최대 30분 간격으로도 위젯을 갱신합니다. 위젯을 누르면 ShoeLog의 러닝화 관리 화면이 열립니다. 위젯은 로컬 Room 데이터만 사용하며 Health Connect를 백그라운드에서 직접 읽지 않습니다.
